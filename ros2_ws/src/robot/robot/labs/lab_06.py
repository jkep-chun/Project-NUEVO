...
#from robot.navigation import NavigationMixin
...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        # -- DRIVING -------------------------------------------------------
            ...
            traffic_light_color = find_traffic_light_color(robot)
            time_since_green = now - last_green_at
            timeout_reached = time_since_green > DRIVE_TIMEOUT_SEC
            if robot.get_detections("stop sign"):
                state = "WATCHING"
                print("[FSM] DRIVING - Stopping due to stop sign")
            elif traffic_light_color == "red":
                state = "WATCHING"
                print("[FSM] DRIVING - Stopping due to red light")
            elif timeout_reached:
                state = "WATCHING"
                print("[FSM] DRIVING - Stopping due to timeout")
            else:
                if traffic_light_color == "green":
                    last_green_at = now
                    print("[FSM] DRIVING - Drive timeout extended")
                # robot.set_velocity(BODY_VELOCITY, 0.0)
                print(f"[FSM] DRIVING - Still driving, {time_since_green} s since green")
